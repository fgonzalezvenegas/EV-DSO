...
#%% Comparing PV installed cap
folder_grids = r'c:\user\U546416\Documents\PhD\Data\MVGrids\Boriette\PPGrid\\'
gridname1 =  'res_grid_equi_2050.json'#'res_grid_equi_2050/res_grid_roof_2.json'
gridname2 =  'res_grid_roof_2.json'#'res_grid_equi_2050/res_grid_roof_2.json'
net = pp.from_json(folder_grids + gridname1)
...
ax = axs[0]
plt.sca(ax)
rt = conso_prod_iris.PV_Commercial_MW.loc[iris_ss] + conso_prod_iris.PV_Home_MW.loc[iris_ss] + conso_prod_iris.PV_farm_MW.loc[iris_ss]
colorsrt = cmap(rt/scale)
util.plot_polygons(polys, ax=ax, color=colorsrt, edgecolor='darkgrey', linestyle='--')
plot_lines(net.line_geodata, col='coords', ax=ax, color='k', linewidth=0.3)
plt.tick_params(left=False,
...
# PLOTTING 2nd grid
net = pp.from_json(folder_grids + gridname2)
...
# Adding zoom to Brive area
az2 = plt.axes([.815, .58, 0.15, .3])
util.plot_polygons(polys, color=colorsrt, edgecolor='darkgrey', linestyle='--', ax=az2)
plot_lines(net.line_geodata, col='coords', ax=az2, color='k', linewidth=0.3)
plt.xticks([])
plt.yticks([])
# setting zoom
...
plt.savefig(r'c:\user\U546416\Pictures\Boriette\PV\Installed_cap_Conti_vs_Syn.pdf'.format(gridname[:-5]))
plt.savefig(r'c:\user\U546416\Pictures\Boriette\PV\Installed_cap_Conti_vs_Syn.jpg'.format(gridname[:-5]), dpi=300)
...
#%% Comparing PV capacity per feeder
    
gridname =  'res_grid_equi_2050.json'#'res_grid_equi_2050/res_grid_roof_2.json'    
net = pp.from_json(folder_grids + gridname)
PV_feeder_conti = pd.DataFrame([net.bus.Feeder[net.sgen.bus].values, net.sgen.p_mw], index=['Feeder','PV']).T.groupby('Feeder').PV.sum()
gridname =  'res_grid_roof_2.json'
net = pp.from_json(folder_grids + gridname)
PV_feeder_syn = pd.DataFrame([net.bus.Feeder[net.sgen.bus].values, net.sgen.p_mw], index=['Feeder','PV']).T.groupby('Feeder').PV.sum()


f, axs = plt.subplots(1,2, sharey=True)
plt.sca(axs[0])
plt.bar(PV_feeder_conti.index, PV_feeder_conti.values)
plt.xticks(rotation=90)
plt.ylabel('Power [MW]')
plt.legend()
plt.title('(a) Continuity', y=-.25)
plt.yticks([0,5,10,15,20])
plt.sca(axs[1])
plt.bar(PV_feeder_syn.index, PV_feeder_syn.values)
plt.xticks(rotation=90)
plt.title('(b) Synergies', y=-.25)

f.set_size_inches(7.57,3.3)
plt.tight_layout()

plt.savefig(r'c:\user\U546416\Pictures\Boriette\PV\PV_conti_syn_feeder.pdf')
plt.savefig(r'c:\user\U546416\Pictures\Boriette\PV\PV_conti_syn_feeder.jpg', dpi=300)
...
plt.savefig(r'c:\user\U546416\Pictures\Boriette\LineLoading\MaxLoading_{}.pdf'.format(case))
plt.savefig(r'c:\user\U546416\Pictures\Boriette\LineLoading\MaxLoading_{}.jpg'.format(case), dpi=300)


#%% Plotting week-long line loading
