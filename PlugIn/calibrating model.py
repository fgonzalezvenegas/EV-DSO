# -*- coding: utf-8 -*-
"""
Created on Thu Dec 10 12:10:50 2020
...
df.index.name = 'alpha'
df.to_csv(output_folder + 'ch_sessions_equalwewds.csv')
...
#alphas.to_csv(output_folder + 'selected_alphas_equalwewds.csv', )
...
for idx in ['small', 'allevs40', 'big' ]:
    ...
    plt.text((yb-lr.params.iloc[0])/lr.params.iloc[1]+dx,yb+dy,
             eq + r2, 
...
haligns = ['left', 'right', 'right']
valigns = ['top', 'bottom', 'bottom']
for j, idx in enumerate(['small', 'allevs40', 'big' ]):
    color = cmap(batt[idx]/100)
    plt.sca(ax[0])
    plt.scatter(ch_sessions[idx][evpl]*7, en_session[idx][evpl], color=color, label='{} BEVs'.format(strs[idx]), alpha=0.7)
    plt.xlabel('Weekly sessions')
    plt.ylabel('Charged energy per session [kWh]')
    plt.grid('--', alpha=0.5)
    plt.xlim(0,12)
    ...
    plt.legend()
    plt.sca(ax[1])
    plt.scatter(ch_sessions[idx][evpl]*7, daily_dist[idx][evpl], color=color, label='{} BEVs'.format(strs[idx]), alpha=0.7)
    plt.xlabel('Weekly sessions')
    plt.ylabel('Daily distance [km]')
    plt.grid('--', alpha=0.5)
    plt.xlim(0,12)
    ...
    dx, dy = -0.2, +0.5
    xb = 10-j
    ys = pred.predicted_mean[xb]
    eq = 'y={:.1f}x+{:.1f}\n'.format(lr.params.iloc[1], lr.params.iloc[0])
    r2 = r'$r^2$={:.2f}'.format(rsq)
    plt.text(xb+dx,ys+dy,
             eq + r2, 
             horizontalalignment=haligns[j],
             verticalalignment=valigns[j])
    print(idx, '\t', eq[:-1], r2)
    plt.legend(loc=4)
    f.tight_layout()