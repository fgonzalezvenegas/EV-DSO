...
Script that iterates does sensitivities
...
commuter_params_LP = dict(arrival_departure_data_wd=adwd_LP,
                          ...
                          alpha=n_proba_low)
commuter_params_EN = dict(arrival_departure_data_wd=adwd_LP,
                          arrival_departure_data_we=adwe_LP,
                          alpha=n_proba_low)
...
##%% Save data
folder = r'C:\Users\u546416\AnacondaProjects\EV-DSO\FlexTenders\EnergyPolicy\Sensi\\'
filehead = 'full_extrapenalties_'


sensitivities = dict(batt_size=[25,50,75], # i include 50 to have a base case
                     charging_power=[3.5,11],
                     tou_ini=[23])#,
#                     target_soc=[1])
#sensitivities = dict(target_soc=[1])
...
#                    print('\tFlexibility profs, t={:.2f} seg'.format(tt[-1]-tt[-2]))
        ...
        #                                    (V1G_bids, V1G_payments, V1G_und, V2G_bids, V2G_payments, V2G_und))
                                    ...
                                    for p in penalty_values:
                                        ...
                                        statsp = fpf.get_stats(V2G_payments, **fpf_params)
                                        stats_VxG['v2g', nevs_fleet, nact, f, j, k, p, sensi, v] = (statsb + statsp + statsu)
...
#                    print('Number of evs {}, t={:.2f} seg\n'.format(nevs_fleet, tt[-1]-tt[0]))
                ...
                stats_VxG.index.names = ['VxG', 'nevs', 'nactivation', 'service_duration', 'confidence', 
                                         'penalty_threshold', 'penalties', 'sensi_param', 'sensi_val']
                ...
                filename = filehead + nameset + '_' + aw_s + 'VxG_sensi' + sensi + str(v) + '.csv'
